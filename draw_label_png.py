...
def main():
    ...
    for i in range(0, len(count)):
        ...
        if count[i].endswith("jpg"):
            ...
            # 找到全局的类
            class_txt = open("./before/class_name.txt", "r",encoding='UTF-8')
            ...
            with open("./output/" + count[i].split(".")[0] + "_json/label_names.txt", "r") as f:
                ...
                for name in names:
                    ...
                    ans =(index_all * (np.array(img) == index_json))
                    new = new + ans

            new = Image.fromarray(np.uint8(new))
            # 将转变后的得到的新的最终的标签图片保存到make_dataset/bmp文件夹下
            new.save(os.path.join("./bmp", count[i].replace("jpg", "bmp")))
